-- word_of_day_log.accuracy and .completed_at were declared in
-- 022_word_of_day_server_streak.sql but were never actually applied to the
-- live database (confirmed via direct query returning
-- "column word_of_day_log.accuracy does not exist") - re-declaring them here
-- idempotently so this migration is safe to run regardless of whether 022
-- landed.
ALTER TABLE public.word_of_day_log
  ADD COLUMN IF NOT EXISTS accuracy INTEGER,
  ADD COLUMN IF NOT EXISTS completed_at TIMESTAMPTZ;

-- complete_word_of_day_attempt() (022_word_of_day_server_streak.sql) declares
-- RETURNS TABLE(..., attempts INTEGER), which implicitly creates a plpgsql
-- variable named "attempts" alongside the identically-named
-- word_of_day_log.attempts column. The body's
-- `SET attempts = COALESCE(attempts, 0) + 1` is therefore ambiguous and the
-- function has always raised `column reference "attempts" is ambiguous`
-- (42702) whenever it actually runs - it stayed latent only because the
-- backend previously called the newer complete_personalized_word_of_day_attempt
-- RPC first, which schema-qualifies the column correctly. This mirrors that
-- fix in the legacy function so it works as its own fallback too.
