import { syllabifyText, syllabifyWord } from './tagalogSyllabification';

describe('tagalog syllabification', () => {
  test('basic words are hyphenated', () => {
  ...
  test('empty and non-letters', () => {
    expect(syllabifyText('')).toBe('');
    expect(syllabifyText('   ')).toBe('');
    expect(syllabifyText('!')).toBe('!');
  });

  test('syllabifyWord returns syllables array', () => {
